# Compliance issue map + tour-level strategy.
# NOT LEGAL ADVICE. A sourced issue map for licensed counsel to work from.

HEADLINE = """The single most consequential finding in this research is not about campuses at all.

On March 17, 2026 the SEC issued an interpretive release (Nos. 33-11412; 34-105020, File No. S7-2026-09,
effective March 23, 2026) that, for the first time, formally addresses airdrops. It creates a "Covered
Airdrop" concept: airdrops of non-security crypto assets where recipients provide NO money, goods,
SERVICES, or other consideration do not satisfy Howey's "investment of money" element. But the release
expressly "does not extend that conclusion to airdrops for services or other structures that require
recipients to provide value in exchange." Practitioner analysis identifies referrals, social-media tasks,
and promotional activity as disqualifying.

DGD's new model sits on the wrong side of that line on two independent grounds:
  1. Referral rewards are compensation for a SERVICE.
  2. The $21 credit is announced in advance with conditions attached — a PROSPECTIVE, conditioned
     distribution, which the release treats as materially riskier than a retroactive, unannounced snapshot.

The structural trap: the referral component is TOO LITTLE consideration to trigger state anti-pyramid
statutes (which require cash or a purchase) but ENOUGH consideration to break the SEC's airdrop carve-out
(which requires zero, including services). The design gets the worst of both frameworks rather than the
benefit of either.

The good news is that this is also the cleanest available fix, and it is a product decision rather than a
legal one: removing the referral tier — or converting it to a retroactive, unannounced snapshot reward —
is the single highest-leverage change available, because it is exactly what moves DGD from outside the
Covered Airdrop carve-out to inside it."""

PREMISE_CORRECTIONS = [
 ("State anti-pyramid statutes are the top risk",
  "NO — they are the LOWEST risk on this list. All nine states define the offence around a participant giving 'consideration,' defined as payment of money or purchase of goods/services/intangible property. Several EXPRESSLY EXCLUDE recruiting effort (UT, WA, MT, AZ; OR excludes 'personal services'). Colorado requires consideration EXCEEDING $50 — a free program is facially outside its statute. A genuinely zero-cost program falls outside the literal text of all nine.",
  "But treat '$0 forever, no purchase path' as a HARD ARCHITECTURAL CONSTRAINT, not a marketing decision. The moment any paid tier appears, all nine switch on simultaneously — and Utah's is a FELONY statute."),
 ("The CLARITY Act will provide cover",
  "NO — the CLARITY Act IS NOT LAW. It passed the House 294–134 on July 17, 2025 and was placed on the Senate Legislative Calendar June 1, 2026; it still requires 60-vote Senate passage and reconciliation with the Senate Agriculture version. The GENIUS Act (stablecoins) WAS signed July 18, 2025. Do not plan around CLARITY passing before Fall 2026.",
  "Plan for the law as it is in August 2026, not as it may be in 2027."),
 ("The $600 1099 threshold applies",
  "OUT OF DATE. The One Big Beautiful Bill Act §70433, signed July 4, 2025, raised the IRC §§6041 and 6041A information-reporting threshold from $600 to $2,000, effective for payments made in 2026 (inflation-indexed from 2027).",
  "At $21/validation a single validation is far below — but a heavy referrer clears $2,000 in roughly 95 referrals, which is achievable for a motivated campus ambassador. Sub-threshold amounts remain TAXABLE INCOME regardless."),
 ("Washington has a comprehensive privacy act to worry about",
  "NO — Washington has NO comprehensive consumer privacy act, only the My Health My Data Act, which is not on point. Washington is LOW privacy risk here.",
  "The state to actually watch is MONTANA: the MCDPA threshold is 50,000 consumers — the lowest of the nine — and it is plausible if the program scales."),
 ("COPPA is the relevant minors statute",
  "WRONG STATUTE, and it will lull you. COPPA's threshold is UNDER 13 — essentially no undergraduates. The real problem is contract law: a person UNDER 18 can generally disaffirm a contract, meaning a minor can accept the $21, refer friends, and then void the terms of service, arbitration clause, and any clawback.",
  "Dual-enrolled high schoolers are on every campus in all nine states and frequently hold institution-issued .edu addresses — so the .edu path is SPECIFICALLY the path that admits minors. Age-gate at 18 AT THE TABLE."),
 ("FERPA governs your collection of student emails",
  "NO — FERPA binds funded educational institutions, not third parties. A student voluntarily typing their own email at a table is not a disclosure 'from education records' by the institution. The real constraint is the university's own solicitation/AUP policy and any data-sharing agreement.",
  "DO NOT represent to any student or administrator that DGD is 'FERPA compliant' — the statute doesn't apply to you and the claim would itself be a §5 deception risk."),
 ("WSU Spokane is a good target",
  "NO — WSU Spokane is WSU's HEALTH SCIENCES campus (Medicine, Nursing, Pharmacy, Medical Sciences). There is no undergraduate business or CS program there. The business and CS students are at WSU PULLMAN.",
  "Redirect that stop to Pullman, 75 miles south."),
 ("EWU is on semesters",
  "NO — EWU is primarily a QUARTER institution: 'four 10-week terms per year,' with semesters only for specific programs.",
  "Only WSU and Gonzaga are semester schools in Washington."),
 ("UVU has a blockchain/fintech program",
  "NOT SUPPORTED — no fintech, blockchain, or digital-assets program, center, or institute appears anywhere on the Woodbury School of Business site.",
  "UVU is a place to BUILD a club relationship, not to find one."),
 ("CU Boulder's Media Archaeology Lab has a blockchain footprint",
  "NO — the MAL is a historical-computing hardware archive directed out of English/Intermedia. No blockchain research footprint found.",
  "CU Boulder's real blockchain presence is Eric Alston at Leeds, the CYBR/CSCI 5240 course, and the CU Blockchain RSO."),
 ("Arizona campuses have already started Fall 2026",
  "NO — no campus in the dataset has started as of Aug 11, 2026. Earliest starts are ASU and CU Boulder on Aug 20.",
  "There is a 9-day runway before the first campus opens."),
]

ISSUES = [
{
 "n":1,"title":"Referral Compensation / Endless-Chain & Pyramid Law","severity":"LOW (statutory) — but a cliff, not a slope",
 "risk":"All nine state anti-pyramid statutes require the participant to give 'consideration,' defined as cash or a purchase; several expressly exclude time and effort spent recruiting. A genuinely free program falls outside their literal text. The real referral exposure is FEDERAL and UDAP-based. The FTC's Koscot analysis has no consideration element: it asks whether there is a right to receive rewards for recruiting 'unrelated to the sale of the product to ultimate users,' and directs attention to 'the incentives that the compensation structure creates.' DGD has NO product sold to ultimate users at all — in a Koscot frame the entire compensation structure is recruitment. Whether §5 reaches a scheme with zero participant outlay is genuinely unsettled; no enforcement action against a free-to-join, no-purchase referral program was found.",
 "cites":[("Utah Code §76-17-301 to -304","⚠ Former Title 76 Ch. 6a was REPEALED 5/7/2025 and recodified into Ch. 17. Any memo citing 76-6a-101 et seq. cites DEAD LAW. §76-17-301(2): consideration 'does not include… time or effort spent in selling or recruiting.' Conducting = 3rd-degree FELONY.","https://le.utah.gov/xcode/Title76/Chapter17/C76-17_2025050720250507.pdf"),
 ...
 "table":"Keep the program genuinely free — no fee, no purchase, no 'buy in to unlock referrals,' no upsell tier. Colorado's $50 floor is per-participant: a $51 anything destroys it.",
},
{
 "n":2,"title":"Securities & Money Transmission","severity":"⚠ HIGHEST — this should gate the Fall 2026 launch",
 "risk":"See the headline finding. Beyond Howey: state money-transmitter exposure turns on whether USD-denominated redeemable 'credit' is stored value. PEER-TO-PEER TRANSFERABILITY IS THE SINGLE FACT MOST LIKELY TO CONVERT THIS INTO MONEY TRANSMISSION EVERYWHERE. If credit is non-transferable and redeemable only for DGD's own goods, the analysis gets much easier; if it is transferable or cashable, assume licensing exposure in ID/WA/OR/UT/CO/NV/AZ.",
 "cites":[("SEC Release Nos. 33-11412; 34-105020","File No. S7-2026-09, announced Mar 17, 2026, effective Mar 23, 2026. §VII: 'Application of the Howey Test to Certain Crypto Asset Disseminations Known as Airdrops' (A. Airdrops Generally; B. Covered Airdrops; C. Interpretation). Five-category taxonomy: digital commodities, digital collectibles, digital tools, stablecoins, digital securities.","https://www.sec.gov/files/rules/interp/2026/33-11412.pdf"),
 ...
 "table":"Do not launch the campus program until counsel has answered whether the referral tier forfeits the Covered Airdrop carve-out, and whether a restructured retroactive distribution gets back inside it.",
},
{
 "n":3,"title":"KYC / AML / Privacy","severity":"MODERATE — and largely self-inflicted",
 "risk":"A BSA/KYC obligation attaches only if DGD is a money services business — which loops back to Issue 2. IF DGD IS NOT AN MSB, IT HAS NO BSA OBLIGATION, AND COLLECTING GOVERNMENT ID IS A PURE LIABILITY WITH NO OFFSETTING COMPLIANCE BENEFIT. Meanwhile collecting ID documents may pull DGD into the GLBA Safeguards Rule and creates breach exposure.",
 "cites":[("GLBA Safeguards Rule, 16 C.F.R. Part 314","Amended 2023 to add breach notification for non-banking financial institutions. [ATTORNEY] — whether DGD is a 'financial institution' under the Rule is genuinely contestable and fact-dependent.","https://www.ftc.gov/legal-library/browse/rules/safeguards-rule"),
 ...
 "table":"DON'T COLLECT WHAT YOU DON'T NEED. Since the KYC path is optional and the BSA likely doesn't compel it, the cheapest risk reduction available is to NOT SCAN GOVERNMENT ID AT A FOLDING TABLE AT ALL.",
},
{
 "n":4,"title":"The .edu Email Path","severity":"MODERATE — the credential is weaker than it looks",
 "risk":"FERPA does not bind DGD (see Premise Corrections). The real constraint is each university's own solicitation policy and acceptable-use policy. Separately, .edu verification is TRIVIALLY DEFEATED — alumni addresses that never expire, dual-enrollment accounts issued to minors, community-college self-service signup, forwarding aliases. That is a serious problem when the credential gates $21 of transferable value.",
 "cites":[("US Dept. of Education, 'Responsibilities of Third-Party Service Providers under FERPA'","Where PII is disclosed under the school-official exception, 'FERPA still governs its use, and the school or district is responsible for its protection.' The vendor is bound because it is 'under the direct control of the agency or institution.' 34 C.F.R. §99.31(a)(1)(i).","https://studentprivacy.ed.gov/sites/default/files/resource_document/file/Vendor%20FAQ.pdf"),
 ...
 "table":"Treat .edu as a WEAK SIGNAL, NOT A CREDENTIAL. Assume a nontrivial fraction of .edu accounts belong to alumni and to under-18 dual-enrolled students.",
},
{
 "n":5,"title":"Minors","severity":"⚠ HIGH — and the .edu path is what creates it",
 "risk":"A minor can accept the $21, refer friends, and then disaffirm the terms of service, the arbitration clause and any clawback. Dual-enrolled high schoolers are on every campus in all nine states and frequently hold institution-issued .edu addresses.",
 "cites":[("COPPA, 15 U.S.C. §§6501–6506; 16 C.F.R. Part 312","Applies to children UNDER 13 — essentially no undergraduates. Well-established but not independently fetched this session; verify before quoting.",""),
 ...
 "table":"AGE-GATE AT 18, AT THE TABLE, BEFORE THE PHONE COMES OUT — not at 13. Because the Colorado and Oregon provisions are keyed to ACTUAL KNOWLEDGE, a tabling rep who can see they're talking to a high schooler creates knowledge the company then has.",
},
{
 "n":6,"title":"Tax","severity":"LOW severity, but expensive to fix retroactively",
 "risk":"Promotional credit and referral rewards are taxable income to recipients regardless of reporting thresholds. Referral rewards paid for RECRUITING ACTIVITY look like §6041A compensation for services — a 1099-NEC question, not a prize question.",
 "cites":[("One Big Beautiful Bill Act §70433","Signed Jul 4, 2025: raises the IRC §§6041 and 6041A information-reporting threshold from $600 to $2,000, effective for payments made in 2026, inflation-indexed from 2027. Sub-threshold amounts remain TAXABLE.","https://www.littler.com/news-analysis/asap/tax-bill-changes-1099-reporting-thresholds"),
...
},
{
 ...
 "table":"Put '#ad' / 'I get paid if you sign up' in the ambassador contract as a MATERIAL TERM WITH A CLAWBACK, provide pre-written disclosure copy, and spot-check ambassador social posts. Ban unqualified 'free money,' 'risk-free,' and any statement about future token value outright.",
},
]
...
# The compliant booth model, derived from MSU Catapalooza's express permissions.
BOOTH_MODEL = [
 ("Age-gate at 18 before anything else","Colorado and Oregon minors provisions are keyed to ACTUAL KNOWLEDGE. A rep who can see they're talking to a high schooler creates knowledge the company then has."),
 ("Educate at the table; do not transact at the table","MSU Catapalooza expressly permits 'informational items, coupons, hand-outs, freebies, company contact information' and collecting contact info for post-event follow-up — and expressly bans monetary exchange. Weber State separately bans having students 'sign any kind of contract for services on site.' CSU bans 'requesting credit card information, Venmo or similar app info.' The intersection of these rules IS your national booth policy."),
 ("Capture an email; complete validation off-campus, off-clock","This single design choice makes the booth compliant at MSU, Weber, CSU and everywhere stricter, and removes the CARD Act pattern-match that alarms administrators."),
 ("Re-confirm interest after the event before any transaction — and never add anyone to a mailing list",
  "Montana State's Catapalooza rules, verbatim: \"If you collect contact information from potential customers interested in purchasing goods or services, you must contact the potential customer after Catapalooza re-confirming their interest before any transactions take place\" and \"do not add a potential customer to any mailing list without their specific consent.\" This is the strictest lead-handling rule found, so it sets the national standard: one follow-up that re-confirms interest, then nothing unless they opt in."),
 ("No government ID scanning at a folding table","If DGD is not an MSB, there is no BSA obligation compelling it — so it is pure liability with no offsetting benefit."),
 ("No raffles anywhere","Prohibited at Boise State (Idaho gambling law, up to $10,000), university-wide at UArizona, and for ASU departments per an Arizona AG determination."),
 ("Ambassadors disclose the material connection every time","FTC Endorsement Guides, 16 C.F.R. Part 255. Make it a contract term with a clawback and provide pre-written copy."),
 ("Never say 'free money' or 'risk-free,' never forecast token value","FTC Act §5 deception risk, and the fastest route to a campus complaint."),
 ("Stay behind the table; no approaching passers-by","WSU: distribution 'by means of accosting, confronting, detaining, or waylaying individuals or by hawking is prohibited.' UW Wyoming: 'All the members of your group must stay behind the table.' Weber: no 'aggressive sales tactics.'"),
 ("Bring the written approval to the table","Most policies make the individual staffing the booth the person asked to leave. UArizona: 'Unapproved vendors are prohibited on campus' — verify approval status by calling (520) 626-2630."),
]

INSURANCE = [
 ("University of Oregon","$1M Commercial General Liability / $2M aggregate, UO named as additional insured. TULIP available.","https://safety.uoregon.edu/event-insurance-tenants-and-facility-users"),
 ("UNLV","Combined single limits of liability of at least $1,000,000, naming the Board of Regents as additional insured, plus hold-harmless/indemnification.","https://www.unlv.edu/campuslife/scheduling-guidelines"),
 ("Colorado School of Mines","$1M per accident bodily injury; $3M per accident property damage. URMIA-TULIP available.","https://ces.mines.edu/external/"),
 ("Western Washington","May require an advance deposit, bond, or insurance (POL-U1000.06).","https://policy.wwu.edu/POL-U1000.06-Scheduling-of-Facilities-by-Off-Campus-Groups"),
 ("University of Arizona","Certificate required ONLY for vendors providing food/beverage samples or displaying a vehicle — DGD would likely be exempt.","https://www.asuatoday.arizona.edu/vendor-fair"),
 ("ASU","Events involving non-university individuals require ≥2 weeks' notice due to added approvals, INSURANCE and fees.","https://public.powerdms.com/ASU/documents/1560623"),
]
