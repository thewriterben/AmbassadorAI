...
export function createApp(db: Db, bank: QuestionBank) {
  ...
  const miniGames = new Set(['pillar_sort', 'design_or_myth', 'chain_builder', 'coin_quest', 'passage']);
  ...
  app.post('/v1/mini/:game', auth, async (c) => {
    ...
    if (status === 'ok' && rounds < config.mini.rewardedRoundsPerDay) {
      if (game === 'coin_quest') {
        ...
        // right = stars (1-3), extra = eras reached (0-9). Every run lands,
        // so every run pays something — that is the point of the design, and
        // an XP formula that paid nothing for a short landing would undo it.
        // Worth less than a Coin Quest level because a passage is about 70
        // seconds and a level is several minutes.
        //
        // The full-passage bonus keys off stars, not extra: two stars IS the
        // whole passage, whereas `extra` counts eras REACHED, which hits 9
        // the moment the coin enters 2009 rather than when it flies out the
        // other side.
        //
        // The coin score adds a small bounded bonus on top (config.mini
        // .scoreXp). It is client-reported like everything else here, so it
        // is capped twice — the score itself and the XP it can turn into —
        // and it never changes what the stars pay.
