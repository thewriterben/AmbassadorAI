...
- **`ARCADE_API`.** Not yet passed to the AAR build, so the embedded arcade
  runs offline and shows its OFFLINE chip. Add
  `--dart-define=ARCADE_API=https://...` to `build_aar.cmd` once the backend
  is hosted. **Superseded by `AUDIT-RC-2026-09-20.md` RC1:** without the
  define the arcade does not merely run offline, it registers against
  `http://localhost:8787` at every ticker launch and trusts whatever answers.
  Build the embed with `DGD_DEMO=true` until a backend exists.
